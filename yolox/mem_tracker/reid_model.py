        ...
        for k, v in net.state_dict().items():
            ...
            if k in h5f:
                ...
                if v.size() != param.size():
                    print(f'Inconsistent shape: {v.size()}, {param.size()}')
                else:
                    v.copy_(param)
            else:
                print.warning(f'No layer: {k}')
            
        epoch = h5f.attrs['epoch'] if 'epoch' in h5f.attrs else -1

        if not load_state_dict:
            if 'learning_rate' in h5f.attrs:
...
class Model(nn.Module):
    def __init__(self, n_parts=8):
        ...
        for i in range(self.n_parts):
            setattr(self, f'linear_feature{i+1}', nn.Linear(512, 64))
    ...
    def forward(self, x):
        ...
        for i in range(self.n_parts):
            ...
            linear_features.append(
                getattr(self, f'linear_feature{i+1}')(pooled_feature.view(pooled_feature.size(0), -1))
            )
